...
def manage_private_volumetypes(
    ...
    for volume_type_name in all_volume_types:
        if not volume_type_name.startswith(f"{domain.name}-"):
            continue

        volume_type = configuration.os_cloud.block_storage.get_type(volume_type_name)
